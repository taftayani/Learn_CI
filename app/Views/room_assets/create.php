...
<head>
    ...
    <title>Create Room-Asset Relation - Asset Assessment System</title>
        ...
        <div class="row">
            ...
            <div class="col-md-9">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2><i class="fas fa-link"></i> Create Room-Asset Relation</h2>
                    <a href="/room-assets" class="btn btn-secondary">
                        <i class="fas fa-arrow-left"></i> Back to Relations
                    </a>
                </div>

                <div class="card">
                    <div class="card-body">
                        <form method="post" action="/room-assets/create">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="room_id" name="room_id" required>
                                            ...
                                            <?php foreach ($rooms as $room): ?>
                                                <option value="<?= $room['id'] ?>" <?= isset($selectedRoomId) && $selectedRoomId == $room['id'] ? 'selected' : '' ?>>
                                                    <?= esc($room['name']) ?> - <?= esc($room['location']) ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                        ...
                                        <select class="form-select" id="asset_id" name="asset_id" required>
                                            ...
                                            <?php foreach ($assets as $asset): ?>
                                                <option value="<?= $asset['id'] ?>">
                                                    <?= esc($asset['name']) ?> - <?= esc($asset['category']) ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                <a href="/room-assets" class="btn btn-secondary">Cancel</a>
                                <button type="submit" class="btn btn-primary">
                                    <i class="fas fa-plus"></i> Create Relation
                                </button>
                            </div>
                        </form>
                    </div>
